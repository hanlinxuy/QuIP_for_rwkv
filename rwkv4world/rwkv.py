    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                quantizers["rwkv.blocks.%d.%s" % (i, name)] = (
...
def rwkv_pack(model, quantizers, wbits, groupsize, save_path):
    ...
    torch.save(
        [{key: None for key in quantizers.keys()}, model.state_dict()], save_path
    )
    print("Done.")
...
if __name__ == "__main__":
    ...
    dataloader, testloader = get_wikitext2_rwkv(
        model=args.model, nsamples=args.nsamples, seed=args.seed, seqlen=model.seqlen
    )
