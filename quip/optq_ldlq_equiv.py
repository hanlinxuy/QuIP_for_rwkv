...
if __name__ == "__main__":
    ...
    gptq_method.quantizer.configure(
        wbits,
        perchannel=True,
        sym=False,
    ...
    ldl_method.configure("ldl_gptqequiv", wbits, npasses=1, unbiased=False)
    ldl_method.quantizer = Quantizer()
    ldl_method.quantizer.configure(
        wbits, perchannel=True, sym=False, qfn="a", mse=False
    )
    ldl_method.preproc(
        preproc_gptqH=False,
        percdamp=0,
    ...
    near_method.quantizer.configure(
        wbits, perchannel=True, sym=False, qfn="a", mse=False
    )
    near_method.preproc(
        preproc_gptqH=False,
    ...
    print(
        f"frac less than 1e-6 apart: {torch.sum((layer_gptq.weight - layer_ldl.weight).abs() < 1e-6) / (layer.H.shape[0]*layer.H.shape[1]):.3f}"
    )
    print(
        f"frac less than 1e-3 apart: {torch.sum((layer_gptq.weight - layer_ldl.weight).abs() < 1e-3) / (layer.H.shape[0]*layer.H.shape[1]):.3f}"
    )
    ...
    print(
        f"frac less than 1e-6 apart: {torch.sum((layer_gptq.weight - layer_near.weight).abs() < 1e-6) / (layer.H.shape[0]*layer.H.shape[1]):.3f}"
    )
    print(
        f"frac less than 1e-3 apart: {torch.sum((layer_gptq.weight - layer_near.weight).abs() < 1e-3) / (layer.H.shape[0]*layer.H.shape[1]):.3f}"
    )

    print("=========== Time and Proxy ==================")

    print(
        f"gptq took {t1-t0:.3f} seconds, ldl took {t2-t1:.3f} seconds, near took {t3-t2:.3f} seconds"
    )
    print(
        f"gptq proxy is {gptq_method.error:.3e}, ldl proxy is {ldl_method.error:.3e}, near proxy is {near_method.error:.3e}"
    )
