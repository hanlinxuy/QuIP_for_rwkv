...
class OPTClass(BaseLM):
    ...
    def _model_logits_on_dataset(self, dataset_inps):
        ...
        if self.model.model.decoder.final_layer_norm is not None:
            self.model.model.decoder.final_layer_norm = (
                self.model.model.decoder.final_layer_norm.to(self.device)
            )
        if self.model.model.decoder.project_out is not None:
            self.model.model.decoder.project_out = (
                self.model.model.decoder.project_out.to(self.device)
            )
        ...
        for i in tqdm(range(nsamples), desc="Last Layer"):
            ...
            batch_logits = F.log_softmax(
                self.model.lm_head(hidden_states)[0][:, :, :50272], dim=-1
            ).cpu()
